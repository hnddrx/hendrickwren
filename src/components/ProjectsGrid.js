...
const ProjectsGrid = ({
  ...
  return (
    ...
    >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
            {projectsToShow.map((project, index) => (
              ...
              >
                ...
                {/* Live iframe preview */}
                <div className="relative overflow-hidden h-56 sm:h-64 lg:h-72 rounded-xl">
                  <iframe
                    src={project.link} // <-- replaced image with live iframe
                    title={project.title}
                    className="w-full h-full border-0 rounded-xl transition-all duration-700 group-hover:scale-105 group-hover:brightness-110"
            ...
            <div className="flex justify-center mt-12">
              ...
              >
                <span className="relative z-10 flex items-center gap-2">
                  {showAll ? 'Show Less Projects' : `See More Projects (${displayedProjects.length - 3} more)`}
                </span>
              </button>
            </div>
          )}
     ...
     <style jsx>{`
       @keyframes particle { 0%, 100% { transform: translate(0, 0); opacity: 0; } 10% { opacity: 1; } 90% { opacity: 1; } 100% { transform: translate( ${Math.random() * 200 - 100}px, ${Math.random() * 200 - 100}px ); opacity: 0; } } .animate-particle { animation: particle linear infinite; } @keyframes slideUp { from { opacity: 0; transform: translateY(40px); } to { opacity: 1; transform: translateY(0); } } @keyframes float { 0%, 100% { transform: translate(0, 0) rotate(0deg); } 33% { transform: translate(30px, -30px) rotate(5deg); } 66% { transform: translate(-20px, 20px) rotate(-5deg); } } @keyframes bounceSubtle { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-2px); } } @keyframes bounceSlow { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-10px); } } @keyframes pulseSlow { 0%, 100% { opacity: 0.3; } 50% { opacity: 0.6; } } .animate-slide-up { animation: slideUp 0.6s ease-out backwards; } .animate-float { animation: float 20s ease-in-out infinite; } .animate-float-delayed { animation: float 20s ease-in-out infinite; animation-delay: -10s; } .animate-bounce-subtle { animation: bounceSubtle 2s ease-in-out infinite; } .animate-bounce-slow { animation: bounceSlow 3s ease-in-out infinite; } .animate-pulse-slow { animation: pulseSlow 3s ease-in-out infinite; } .animate-fade-in { animation: fadeIn 0.8s ease-out; } @keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } }
      `}</style>
    </section>
  );
};
